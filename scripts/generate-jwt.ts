...
class JWTGenerator {
  ...
  private validateInput(options: Partial<GenerateTokenOptions>): GenerateTokenOptions {
    const errors: string[] = [];
  ...
  generateToken(options: Partial<GenerateTokenOptions>): string {
  ...
  displayTokenInfo(token: string, payload: TokenPayload, expiresIn?: string): void {
...
Usage:
  ...
  yarn generate-jwt --id <user_id> --username <username> --type <user|admin> [options]
...
function parseArgs(): Partial<GenerateTokenOptions> | null {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--username':
        ...
        break;
      case '--type':
        options.type = args[++i] as 'user' | 'admin';
        break;
      case '--expires':
